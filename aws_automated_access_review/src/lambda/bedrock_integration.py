...
def invoke_claude_model(bedrock, prompt):
    """
    Invoke the Amazon Claude model via Bedrock Messages API.
    ...
    using the modern Messages API format.
    ...
    # Using Claude Sonnet 4 with Messages API for comprehensive text generation capabilities
    model_id = "us.anthropic.claude-sonnet-4-6"  # Amazon Bedrock model identifier

    # Step 2: Construct the messages using the Messages API format
    # The messages array follows the new format with system message and user message
    request_body = {
        "messages": [
            {
                "role": "user",
                "content": (
                    ...
                    f"Generate a concise, professional security report based on the following "
                    f"findings:\n\n{prompt}\n\n"
        ...
        "max_tokens": 4096,  # Maximum response length (roughly 3000 words)
...
def extract_narrative_claude(response):
    """
    Extract the generated narrative from the Bedrock Messages API response.
    ...
    """
    try:
        # For Claude Messages API, the generated text is in the "content" array
        # We extract the text from the first content block
